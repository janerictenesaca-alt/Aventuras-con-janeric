...
X_DOBLE = 19.0
BORDE_DOBLE = 12.0                # metal entre el hueco y el borde de abajo de la placa (pedido: 10–15 mm)
Z_DOBLE = BORDE_DOBLE + D_DOBLE / 2    # 24,5
...
# ------------------------------------------------------------------ 01r refuerzo placa frontal ↔ casquillo (4 iguales, sutiles)
REF_T = 8.0                       # grosor de la cartelita (a lo largo del tubo)
REF_L = 16.0                      # largo sobre la placa (desde el borde): sutil, sin tocar la aleta de afuera
REF_FONDO = 24.0                  # cuánto baja por atrás, pegada al tubo


def perfil_refuerzo():
    """Cartelita en el corte del tubo (u = hacia adentro de la placa desde el eje del tubo, y): abraza el tubo por atrás."""
    r = CAS_OD / 2
    u0 = OFF_BORDE
    p = Polygon([(u0 + REF_L, Y_FRENTE[0]), (u0 - 20, Y_FRENTE[0]), (u0 - 20, Y_FRENTE[0] - REF_FONDO), (r - 3.0, Y_FRENTE[0] - REF_FONDO)])
    p = p.difference(Point(0, 0).buffer(r + 0.2, quad_segs=96))
    p = p.buffer(-1.5, quad_segs=8).buffer(1.5, quad_segs=8)
    return max(getattr(p, "geoms", [p]), key=lambda q: q.area)


REFUERZO = perfil_refuerzo()
...
LUZ_PUENTE = 20.0                 # espacio entre la placa 01 y el puente: por ahí baja el mosquetón de los 2 huecos
PUENTE_FRENTE = Y_FRENTE[0] - LUZ_PUENTE          # −13,9: borde de adelante del puente
C_HUECO = (0.0, round(PUENTE_FRENTE - 12.0 - D_MOSQ / 2, 1))   # hueco C con 12 mm de metal adelante
C_BORDE = 14.0                    # metal alrededor del hueco C hacia atrás
PUENTE_Y = (C_HUECO[1] - D_MOSQ / 2 - C_BORDE, PUENTE_FRENTE)


def puente_central():
    """Placa acostada entre las aletas de adentro, con el hueco C. Borde de atrás redondo alrededor del hueco."""
    r = D_MOSQ / 2 + C_BORDE
    p = unary_union([box(-X_ALETA_INT, C_HUECO[1], X_ALETA_INT, PUENTE_FRENTE), Point(*C_HUECO).buffer(r, quad_segs=64),
                     Polygon([(-X_ALETA_INT, C_HUECO[1]), (X_ALETA_INT, C_HUECO[1]), (r, C_HUECO[1] - 6), (-r, C_HUECO[1] - 6)])])
    p = p.intersection(box(-X_ALETA_INT, -200, X_ALETA_INT, PUENTE_FRENTE)).buffer(3, quad_segs=8).buffer(-3, quad_segs=8)
    p = p.intersection(box(-X_ALETA_INT, -200, X_ALETA_INT, PUENTE_FRENTE))
...
# ------------------------------------------------------------------ 17 AHP v10: como el original (1 placa: punta, centro con 4 huecos, 2 patitas) + refuerzo atrás
AHP_T = 25.4                       # placa principal A36 de 1": el pasador 3/8" la cruza de canto
AHP_TR = 12.7                      # refuerzos, pared y argollas A36 1/2"
AHP_U = 52.0                       # luz entre patitas: entra la pata de adentro Ø 50,8
AHP_PATA = (25.4, 150.0)           # ancho y largo de cada patita (y = 0 … 150)
AHP_PIN = (24.0, -AHP_T / 2)       # (y, z) del pasador 3/8" (de canto, al medio del grosor)
AHP_CUERPO_Y = (150.0, 262.0)
AHP_HUECOS = [(sx * 72.0, y) for sx in (-1, 1) for y in (180.0, 232.0)]
AHP_TUBO = (50.8, 6.35, 300.0)     # tubo cuadrado 2" × 1/4": va por atrás, desde la pared hasta el enganche
AHP_TUBO_Y0 = AHP_CUERPO_Y[0] + AHP_TR
AHP_ARG_Y = 78.0                   # argollas a media patita
AHP_ARG_ANG = 10.0                 # inclinadas 10° hacia atrás


def ahp_placa():
    """17b: placa principal (x, y) de una pieza: 2 patitas, centro con 4 huecos Ø 25 en lóbulos."""
    u, a, L = AHP_U / 2, AHP_PATA[0], AHP_PATA[1]
    patas = [box(u, 12.7, u + a, L + 20), box(-u - a, 12.7, -u, L + 20)]
    puntas = [Point(sx * (u + a / 2), 12.7).buffer(a / 2, quad_segs=32) for sx in (-1, 1)]
    cuerpo = box(-(u + a), L, u + a, AHP_CUERPO_Y[1])
    lobulos = [Point(x, y).buffer(12.5 + 13.0, quad_segs=48) for x, y in AHP_HUECOS]
    puente = [box(40, 180, 97.5, 232), box(-97.5, 180, -40, 232)]
    p = unary_union(patas + puntas + [cuerpo] + lobulos + puente)
    p = p.buffer(4, quad_segs=8).buffer(-4, quad_segs=8)
    hs = [(f"A{i + 1}", x, y, 25.0) for i, (x, y) in enumerate(AHP_HUECOS)]
    return Polygon(p.exterior).simplify(0.02), hs, []


def ahp_pared():
    """17d: pared cruzada atrás (x, z) donde empieza el tubo."""
    L = AHP_TUBO[0]
    return box(-L / 2 - AHP_TR, -AHP_T - L, L / 2 + AHP_TR, -AHP_T), [], []


def ahp_cartela():
    """17e: cartela (y, z) a cada lado del tubo, une el tubo con la parte de atrás de la placa (como la gin pole)."""
    y0, y1, zt, zb = AHP_TUBO_Y0, AHP_CUERPO_Y[1] - 4, -AHP_T, -AHP_T - AHP_TUBO[0] + 4
    p = Polygon([(y0, zt), (y1, zt), (y0 + 30, zb), (y0, zb)]).buffer(-3, quad_segs=8).buffer(3, quad_segs=8)
    return Polygon(p.exterior).simplify(0.02), [], []


def ahp_argolla():
    """17f: orejita (u, v): u hacia afuera desde la patita; ventana redondeada para mosquetón o maniota."""
    p = box(0, -31, 50, 31).buffer(-12, quad_segs=12).buffer(12, quad_segs=12).union(box(0, -31, 14, 31))
    ventana = box(16, -14, 34, 14).buffer(4, quad_segs=12)
    return Polygon(p.exterior).simplify(0.02), [], [ventana]


AHP_PIEZAS = {"17b": ahp_placa(), "17d": ahp_pared(), "17e": ahp_cartela(), "17f": ahp_argolla()}
...
D_PUNTA = 22.0
HUECOS_P2 = (51.0, -172.0)        # (±x, y): 12,7 mm de metal al borde, al hueco P y al hueco del tubo
...
def perfil_ala(alfa=GP_ALFA):
    ...
    for sx in (-1, 1):                                                                         # P2: 2 anclajes más, a los lados
        cortes.append(Point(sx * HUECOS_P2[0], HUECOS_P2[1]).buffer(D_PUNTA / 2, quad_segs=48))
