"""Planos del trípode tipo Vortex v9 (placas de corte láser soldadas con TIG), A3 horizontal.

Uso:  python3 v9_doc.py <carpeta_imagenes> <carpeta_fuentes>
Salida:  ../v9/plano/index.html (para imprimir a PDF), ../v9/svg/*.svg (piezas planas a escala 1:1)
         ../v9/web/dibujos.json (dibujos para la página)
...
g.Z_DOBLE = ARM["z_doble"]                 # los 2 huecos quedan por encima del puente (calculado en v9_build)
PUENTE_Z0 = ARM["puente_z0"]               # cara de abajo del puente 04
...
def dib_ala(ancho):
    ...
    v.diametro(0, yP, g.D_PUNTA / 2, 200, "P · Ø 22 (anclaje de la punta)")
    rb = ovalos[-1].bounds
...
def dib_ahp_tubo(ancho):
    ...
    return vista_tubo(largo, L, L - 2 * e, [(largo - 60.0, 16.5, "")], "17a · Espiga de tubo cuadrado 2\" × 2\" × 1/4\"" + fab(1),
                      f"Escala 1:{f((largo + 90) / ancho, 2)}", ancho, arriba_txt="se suelda al yugo 17b", abajo_txt="entra en el enganche del carro",
                      cls="pieza3")
...
LISTA = [
    ...
    ("17a", "AHP · espiga de tubo cuadrado", "Tubo A36 2\" × 2\" × 1/4\" × 300", 1, "17a", AC),
    ("17b", "AHP · yugo (placa en C)", "Acero A36 12,7 mm · láser", 1, "17b", AC),
    ("17c", "AHP · brazo largo", "Acero A36 12,7 mm · láser", 2, "17ci", AC),
    ("17d", "AHP · placa de anclajes", "Acero A36 12,7 mm · láser", 1, "17d", AC),
    ("17e", "AHP · alma de la T", "Acero A36 12,7 mm · láser", 1, "17e", AC),
    ("17f", "AHP · argolla inclinada", "Acero A36 12,7 mm · láser", 2, "17fi", AC),
    ...
    c = f"""<div class="cont" style="grid-template-columns:150mm 1fr">
      <div class="bloque" style="gap:5mm;align-content:space-between">
        <div class="bloque" style="gap:4mm">
          <div class="kick">Juego de planos de fabricación · Rev. E</div>
        ...
        <div class="ficha">
          ...
          <dt>Reemplaza</dt><dd>Rev. D: ya no se usa</dd>
        </div>
      </div>
      <div style="display:grid;grid-template-rows:1fr 1fr;gap:6mm">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4mm;align-items:center">
          <img class="render" src="{img('v9_t')}" style="height:112mm;justify-self:center">
          <img class="render" src="{img('v9_tb')}" style="height:112mm;justify-self:center">
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:6mm;align-items:center">
          <img class="render" src="{img('v9_ciso')}" style="height:74mm">
          <img class="render" src="{img('v9_car')}" style="height:74mm">
        </div>
      </div>
    </div>"""
    hoja("—", "Portada", "—", "", "S/E", c)
...
def hoja_lista():
    ...
    c = f"""<div class="cont" style="grid-template-columns:1fr 128mm">
      <div class="bloque" style="gap:4mm">
        ...
        13 pata de adentro · 14 pie · 15–16 carrete · 17a–f AHP del carro · 18a–h pie con rótula. Se compran: 17 pasadores 3/8", 4 (+1) pasadores 1/2",
      ...
      <div class="bloque" style="gap:4mm">
        ...
        <img class="render" src="{img('v9_t')}" style="height:62mm;justify-self:center">
      </div>
    </div>"""
...
def hoja_armado_cabeza():
    ...
    c = f"""<div class="cont" style="grid-template-columns:1fr 112mm;grid-template-rows:auto auto 1fr;grid-template-areas:'t t' 'f l' 'a l'">
      ...
      <div class="bloque" style="grid-area:l;gap:5mm">{guardar("arm_cab_lado", la.svg("Vista de lado", esc(la)))}
        <img class="render" src="{img('v9_ab')}" style="height:46mm">
        <div class="aviso"><b>Puente 04 y 2 huecos</b>El puente sube a z = {f(PUENTE_Z0)}–{f(PUENTE_Z0 + T)} mm: queda 5 mm por debajo de la gin pole cuando la pata se junta (tope a {f(ARM["giro_max"][0], 0)}°, la gin pole baja hasta z = {f(ARM["zmin_gin"])}). Los 2 huecos Ø 25 de la placa 01 (en vez del corazón) van a z = {f(g.Z_DOBLE)}: por encima del puente, sin taparse.</div>
      </div>
    ...
    c = f"""<div class="cont" style="grid-template-columns:245mm 1fr;grid-template-rows:auto 1fr">
      ...
      <div class="bloque">{guardar("01", v1.svg("01 · Placa frontal (cara lisa)" + fab(1), esc(v1)))}
        <dl class="ficha">
          ...
          <dt>Huecos</dt><dd>2 huecos Ø 25 mm para mosquetón grande (uno suelto en cada uno), en vez del corazón: centros a ±{f(g.X_DOBLE)} mm del centro y a {f(g.Z_DOBLE)} mm del borde de abajo; 13 mm de metal entre los dos. Las otras medidas no cambian.</dd>
...
def hoja_piezas_cabeza_2():
    ...
    c = f"""<div class="cont" style="grid-template-columns:auto auto 1fr;grid-template-rows:auto 1fr auto;column-gap:14mm">
      ...
      <table style="grid-column:1/4"><thead><tr><th>N.º</th><th>Pieza</th><th>Cant.</th><th>Dónde va</th><th>kg c/u</th></tr></thead><tbody>
        ...
        <tr><td class="n">04</td><td>Puente central</td><td class="m">1</td><td>Acostado entre las aletas de adentro, SUBIDO a z = {f(PUENTE_Z0)} … {f(PUENTE_Z0 + T)} mm (5 mm bajo la gin pole cuando se junta), desde la placa 01 hasta {f(g.Y_FRENTE[0] - g.PUENTE_Y[0])} mm hacia atrás. Hueco C Ø 22 mm vertical a {f(g.Y_FRENTE[0] - g.C_HUECO[1])} mm de la placa, con {f(g.C_BORDE)} mm de metal atrás. No tapa los 2 huecos de la placa 01.</td><td class="m">{m2(kg('04'))}</td></tr>
      </tbody></table>
    </div>"""
...
def hoja_gin():
    ...
    c = f"""<div class="cont" style="grid-template-columns:132mm 1fr;grid-template-rows:auto 1fr">
      ...
      <div class="bloque" style="gap:5mm">
        ...
        <dl class="ficha">
          <dt>05 Ala</dt><dd>Plancha 12,7 mm, {m2(kg('05'))} kg. 4 óvalos D, hueco P Ø 22 en la punta (nuevo, punta alargada 45 mm), hueco del tubo y 2 ranuras donde entran las lengüetas de las orejas. Todo hueco con ≥ 12 mm de metal.</dd>
        ...
        <img class="render" src="{img('v9_ciso')}" style="height:52mm">
        </div>
      </div>
    </div>"""
...
def hoja_ahp():
    b17, c17, d17, e17, f17 = (g.AHP_PIEZAS[k] for k in ("17b", "17c", "17d", "17e", "17f"))
    vb = dib_plana(b17[0], b17[1], [], 70)
    vc = dib_plana(c17[0], c17[1], [], 70)
    vd = dib_plana(d17[0], d17[1], [], 84, notas=[(0, 70, 30, 20, "hueco en U: pasa la punta de la pata")])
    ve = dib_plana(e17[0], e17[1], [], 88)
    vf = dib_plana(f17[0], f17[1], [], 44)
    ta = dib_ahp_tubo(150)
    WEB["17a"] = ta
    c = f"""<div class="cont" style="grid-template-columns:auto auto auto 1fr;grid-template-rows:auto auto auto 1fr;column-gap:8mm;row-gap:3mm">
      <div class="bloque" style="grid-column:1/5"><div class="kick">Accesorio aparte · para el vehículo · acero A36 soldado con MIG</div><h2>Poste de enganche AHP (17a–17f)</h2></div>
      {guardar("17b", vb.svg("17b · Yugo (C)" + fab(1), esc(vb)))}
      {guardar("17c", vc.svg("17c · Brazo largo" + fab(2), esc(vc)))}
      {guardar("17f", vf.svg("17f · Argolla inclinada" + fab(2), esc(vf)))}
      <img class="render" src="{img('v9_ahp2')}" style="height:62mm;grid-row:2/4;grid-column:4">
      {guardar("17d", vd.svg("17d · Placa de anclajes" + fab(1), esc(vd)))}
      {guardar("17e", ve.svg("17e · Alma de la T" + fab(1), esc(ve)))}
      <div style="grid-column:3">{ta}</div>
      <dl class="ficha" style="grid-column:1/5">
        <dt>Cómo va</dt><dd>La espiga cuadrada 17a entra DIRECTO en el enganche de 2" del carro (sin adaptador), con su pasador Ø 16 mm (5/8") por el hueco de Ø 16,5 a 60 mm de la punta. Se suelda al yugo 17b. Los 2 brazos 17c salen del yugo con 52 mm de luz: ahí entra la pata de adentro (Ø 50,8) y el pasador 3/8" cruza los brazos y el hueco de 63,5 mm de la pata (la pata gira).</dd>
        <dt>La T</dt><dd>El alma 17e va parada bajo la espiga, por el centro, y la placa de anclajes 17d va acostada bajo los brazos: juntas forman la T de abajo. La placa tiene 4 huecos Ø 25 para mosquetón (≥ 13,5 mm de metal al borde) y un hueco en U para que pase la punta de la pata.</dd>
        <dt>Argollas</dt><dd>2 argollas 17f soldadas afuera de cada brazo, inclinadas 35° hacia arriba, hueco Ø 22 (13 mm de metal). Para maniota o mosquetón.</dd>
        <dt>Soldadura</dt><dd>MIG ER70S-6, filete de 8 mm todo alrededor. Galvanizar o pintar. {m2(sum(kg(p, AC) for p in ("17a", "17b", "17ci", "17cd", "17d", "17e", "17fi", "17fd")))} kg el conjunto.</dd>
      </dl>
    </div>"""
    ...
    indice("17a a 17f · Poste de enganche AHP para el carro (acero)")
...
def hoja_rotula():
    ...
    c = f"""<div class="cont" style="grid-template-columns:auto auto 1fr;grid-template-rows:auto auto 1fr;column-gap:10mm">
      ...
      <img class="render" src="{img('v9_rot')}" style="height:82mm;grid-row:2/4;grid-column:3;justify-self:center">
...
COMPRAS = [
    ...
    ("M5", "Plancha A36 12,7 mm · tubo cuadrado A36 2\" × 1/4\" × 300 · A36 10 y 12 mm", "según plano", "—", "AHP 17, bases 18f, garras 14."),
...
PASOS_TALLER = [
    ...
    ("Cabeza", "Placa 01 boca abajo; 4 aletas paradas en x = ±" + f(g.X_ALETA_INT) + " y ±" + f(g.X_ALETA_EXT) + " mm con un eje de Ø 12 mm por los huecos; puente 04 a " + f(PUENTE_Z0) + " mm del borde de abajo; casquillos con gabarit a " + f(g.ANG) + "°. Puntear, medir, soldar TIG alternando lados."),
...
def svgs_corte():
    ...
    for k, n, q in (("17b", "yugo", 1), ("17c", "brazo", 2), ("17d", "placa_anclajes", 1), ("17e", "alma_T", 1), ("17f", "argolla", 2)):
        p, hs = g.AHP_PIEZAS[k]
        svg_corte(f"{k}_AHP_{n}_A36_12,7mm_x{q}", p, hs, (), f"{k} AHP {n} · A36 12,7 mm · FABRICAR × {q}")
...
if __name__ == "__main__":
    exec(open(os.path.join(AQUI, "v9_cadenas.py")).read())
    hoja_lista()
    hoja_regla()
    hoja_armado_cabeza()
    ...
    svgs_corte()
    total = len(HOJAS)
    html = (f'<!doctype html><html lang="es"><head><meta charset="utf-8"><title>VX-EC Trípode v9 · planos</title>'
