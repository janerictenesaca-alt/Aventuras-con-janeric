"""Trípode tipo Vortex v10: piezas de corte láser, modelo 3D armado (una pieza por nodo) y DXF.

Uso:  python3 v10_build.py   ->  ../v10/
"""
import json
import math
import re
...
import v10_barrido as barr
TOPE_GIRO, ZMIN_GIN = barr.barrido(g, ALFA_D, math.degrees(SOL["A"][1]))
LUZ = 5.0
PUENTE_Z0 = 0.0                                   # puente abajo, separado 20 mm de la placa: los mosquetones bajan por ese espacio
assert ZMIN_GIN - PUENTE_Z0 - T >= LUZ, ZMIN_GIN     # la gin pole no lo toca al juntarse
...
S_REFUERZO = (20.0, 128.0)                     # posición de las cartelitas (desde la boca de arriba del casquillo)


def refuerzo(lado, s_):
    """01r: cartelita entre la cara de atrás de la placa 01 y el casquillo, en el corte del tubo."""
    pts = [(-lado * u, y) for u, y in list(g.REFUERZO.exterior.coords)[:-1]]
    sol = cq.Workplane("XY").polyline(pts).close().extrude(g.REF_T).translate((0, 0, -s_ - g.REF_T / 2))
    return en_casquillo(sol, lado)


for lado, t in ((-1, "izq."), (1, "der.")):
    for k, s_ in zip("ab", S_REFUERZO):
        P.append((f"01r{'i' if lado < 0 else 'd'}{k}", f"Refuerzo placa–casquillo {t} {'arriba' if k == 'a' else 'abajo'}",
                  refuerzo(lado, s_), AZUL, "cabeza", "Al 6061-T6 · 8 mm · láser", 1))
pa, ha = g.ALETA
...
def ahp_partes():
    """17: AHP como el original. Origen: centro del pasador en planta; y hacia el carro; cara de adelante de la placa en z = 0."""
    T1, t = g.AHP_T, g.AHP_TR
    L, e, largo = g.AHP_TUBO
    pb, hb, _ = g.AHP_PIEZAS["17b"]
    placa = extruir(pb, T1, hb).translate((0, 0, -T1))
    placa = placa.cut(cil(g.D38, 200, "x").translate((0, g.AHP_PIN[0], g.AHP_PIN[1])))
    y0 = g.AHP_TUBO_Y0
    tubo_c = cq.Workplane("XZ").rect(L, L).rect(L - 2 * e, L - 2 * e).extrude(-largo).translate((0, y0, -T1 - L / 2))
    tubo_c = tubo_c.cut(cil(16.5, 200, "x").translate((0, y0 + largo - 60, -T1 - L / 2)))
    pd, _, _ = g.AHP_PIEZAS["17d"]
    pared = placa_xz(pd, t, y_frente=y0)
    pe_, _, _ = g.AHP_PIEZAS["17e"]
    out = [("17a", "Tubo cuadrado 2\" × 1/4\" (punta al enganche)", tubo_c, ACERO_C, "Acero A36"),
           ("17b", "Placa principal (centro + 2 patitas)", placa, NARANJA, "Acero A36 25,4"),
           ("17d", "Pared de refuerzo cruzada", pared, NARANJA, "Acero A36 12,7")]
    for sx, n in ((-1, "izq."), (1, "der.")):
        x0 = L / 2 if sx > 0 else -L / 2 - t
        out.append((f"17e{'i' if sx < 0 else 'd'}", f"Cartela de atrás {n}", placa_yz(pe_, t, (), x0), NARANJA, "Acero A36 12,7"))
    pa_, _, ca_ = g.AHP_PIEZAS["17f"]
    u_ = g.AHP_U / 2 + g.AHP_PATA[0]
    for sx, n in ((-1, "izq."), (1, "der.")):
        arg = extruir(pa_, t, (), ca_).translate((0, 0, -t / 2))
        arg = arg.rotate((0, 0, 0), (0, 1, 0), g.AHP_ARG_ANG).translate((u_ + 1.3, g.AHP_ARG_Y, -T1 / 2))   # 10° hacia atrás
        if sx < 0:
            arg = arg.mirror("YZ")
        out.append((f"17f{'i' if sx < 0 else 'd'}", f"Orejita inclinada 10° {n}", arg, NARANJA, "Acero A36 12,7"))
    pata = PI_.rotate((0, 0, 0), (0, 0, 1), 90).translate((0, 0, g.H_BOCA)).rotate((0, 0, 0), (1, 0, 0), -120)
    out.append(("17x", "Pata de adentro en el AHP", pata.translate((0, g.AHP_PIN[0], g.AHP_PIN[1])), PLATA, ""))
    out.append(("17p", "Pasador del AHP 3/8\" × 4\" de agarre", pasador(9.5, 105.0).rotate((0, 0, 0), (0, 0, 1), 90)
                .translate((0, g.AHP_PIN[0], g.AHP_PIN[1])), ORO, "Se compra"))
    return out
...
glb([(f"n{i:02d}_{it[0]}", it[2], it[3]) for i, it in enumerate(MUNDO)], os.path.join(OUT, "tripode_v10.glb"))
glb([(f"n{i:02d}_{it[0]}", it[2], it[3]) for i, it in enumerate(MUNDO_B)], os.path.join(OUT, "tripode_v10_tubo_arriba.glb"))
asm = cq.Assembly(name="tripode_v10")
for i, it in enumerate(MUNDO):
    asm.add(it[2], name=f"n{i:02d}_{it[0]}", color=it[3])
asm.export(os.path.join(OUT, "tripode_v10.step"))
...
cab = cq.Assembly(name="cabeza_v10")
for pid, nom, s_, col, grp, mat, q in P:
...
glb([(f"c_{it[0]}", it[2], it[3]) for it in P] + [(f"g_{it[0]}", gin_cab(it[2]), it[3]) for it in GIN],
    os.path.join(OUT, "cabeza_v10.glb"), 0.03)
cab.export(os.path.join(OUT, "cabeza_v10.step"))
glb([(f"c_{it[0]}", it[2], it[3]) for it in P if it[4] == "cabeza"], os.path.join(OUT, "aframe_v10.glb"), 0.03)
glb([(f"a_{it[0]}", it[2], it[3]) for it in ACC], os.path.join(OUT, "accesorios_v10.glb"), 0.06)
acc = cq.Assembly(name="accesorios_v10")
for it in ACC:
    acc.add(it[2], name=f"a_{it[0]}", color=it[3])
acc.export(os.path.join(OUT, "accesorios_v10.step"))
...
DXF = [
    ...
    ("17b_AHP_placa_principal", "17b Placa principal del AHP", g.AHP_PIEZAS["17b"][0], g.AHP_PIEZAS["17b"][1], [], 1, "acero_A36_25,4mm"),
    ("17d_AHP_pared", "17d Pared de refuerzo del AHP", g.AHP_PIEZAS["17d"][0], [], [], 1, "acero_A36_12,7mm"),
    ("17e_AHP_cartela", "17e Cartela de atrás del AHP", g.AHP_PIEZAS["17e"][0], [], [], 2, "acero_A36_12,7mm"),
    ("17f_AHP_orejita", "17f Orejita del AHP", g.AHP_PIEZAS["17f"][0], [], g.AHP_PIEZAS["17f"][2], 2, "acero_A36_12,7mm"),
    ("01r_refuerzo_placa_casquillo", "01r Refuerzo placa-casquillo", g.REFUERZO, [], [], 4, "Al6061-T6_8mm"),
